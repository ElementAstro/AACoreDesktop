...
QWidget* T_CameraPage::createInfoTab() {
    ...
    // 添加描述
    QLabel *descriptionLabel = new QLabel("描述: A basic simulator to generate random noise for a specific median or load in an image that will be displayed on capture");
...
QWidget* T_CameraPage::createSettingCard(const QString& title, const QString& iconPath, 
                                         const QString& description, const QString& settingName, 
...
QChartView *T_CameraPage::createTemperatureChart() {
  ...
  for (int i = 0; i < 8; ++i) {
    series->append(startTime.addDays(i).toMSecsSinceEpoch(), rand() % 100 - 50);
  }
