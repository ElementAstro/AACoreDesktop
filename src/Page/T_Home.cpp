...
T_Home::T_Home(QWidget *parent) : ElaScrollPage(parent) {
  ...
  ElaText *flowLayoutText = new ElaText("天文摄影工具", this);

  // 天文摄影工具卡片
  ElaPopularCard *starStackerCard = new ElaPopularCard(this);
  connect(starStackerCard, &ElaPopularCard::popularCardButtonClicked, this,
          [=]() {
  ...
  starStackerCard->setDetailedText("强大的星轨叠加工具，自动对齐和合并多张长曝"
                                   "光图像，创造出令人惊叹的星轨效果。");

  ElaPopularCard *deepSkyCard = new ElaPopularCard(this);
  ...
  ElaPopularCard *planetaryCard = new ElaPopularCard(this);
  ...
  ElaPopularCard *processingCard = new ElaPopularCard(this);
  ...
  processingCard->setDetailedText("强大的后期处理工具，包括去噪、锐化、色彩校正"
                                  "等功能，让您的天文照片更加出色。");
  ...
  // 滚动区域中的卡片
  ElaAcrylicUrlCard *tutorialCard = new ElaAcrylicUrlCard(this);
  ...
  ElaAcrylicUrlCard *communityCard = new ElaAcrylicUrlCard(this);
  ...
  QHBoxLayout *cardScrollAreaWidgetLayout =
      new QHBoxLayout(cardScrollAreaWidget);
  ...
  ElaMenu *viewMenu = _homeMenu->addMenu(ElaIconType::Cubes, "查看");
...
void T_Home::mouseReleaseEvent(QMouseEvent *event) {
  switch (event->button()) {
  case Qt::LeftButton: {
    // ElaMessageBar::success(ElaMessageBarType::TopRight, "Success", "Never
    // Close Your Eyes", 2500);
    // ElaMessageBar::success(ElaMessageBarType::TopRight, "Success", "Never
    // Close Your Eyes", 1500);
    break;
  }
  case Qt::RightButton: {
