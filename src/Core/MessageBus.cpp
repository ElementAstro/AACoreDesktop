...
MessageBus::MessageBus(QObject *parent)
    : QObject(parent), m_webSocketClient(new WebSocketClient(this)),
...
void MessageBus::loadPersistedMessages() {
  ...
  while (query.next()) {
    ...
    msg.data =
        QJsonDocument::fromJson(query.value("data").toByteArray()).toVariant();
...
void MessageBus::processMessageQueue() {
  while (!m_messageQueue.isEmpty()) {
    ...
    QByteArray data = QJsonDocument(jsonMessage).toJson(QJsonDocument::Compact);
    ...
    case HTTP:
      ...
      QJsonObject jsonMessage = variantMapToJsonObject(jsonMessageMap);
