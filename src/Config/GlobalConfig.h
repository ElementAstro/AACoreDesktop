...
public:
  ...
  template <typename T> T useStore(const QString &storeName);
...
private:
  ...
  std::unordered_map<
      QString, std::unordered_map<void *, std::function<void(const QString &)>>>
      m_subscribers;
...
template <typename T> T GlobalConfig::useStore(const QString &storeName) {
  if (!m_stores.contains(storeName)) {
